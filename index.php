...
function sniffTeam($refTeam,$steps){
	...
	$refTeam = mysql_real_escape_string($refTeam);
...
$con = null; // Open SQL Connection here
if (!$con)
  {
...
if(isset($_GET['start']) && isset($_GET['end'])){
	if(is_numeric($_GET["start"]) && is_numeric($_GET["end"])){
		...
		$query = mysql_query("SELECT * FROM `teams` WHERE `teamnumber` = '".$start."' OR `teamnumber` = '".$end."';") or die(mysql_error());
...
if(!isset($message) && isset($_GET['start']) && isset($_GET['end'])){
	...
	$marker = mysql_real_escape_string($endTeam);
	for($i=0;$i<=$teams[$endTeam];$i++){
		...
		if($i!=$teams[$endTeam]){
		  $matchId = mysql_real_escape_string($matchid[$marker]);
		  $query = mysql_query("SELECT `events`.`name` as `eventTitle`, `events`.`year` as `year`, `matches`.`matchname` as `matchTitle`, `matches`.`matchnumber` as `number`, `events`.`eventshort` as `short` FROM `matches` LEFT JOIN `events` ON `matches`.`event` = `events`.`eventid` WHERE `matchid` = '".$matchId."'") or die(mysql_error());
